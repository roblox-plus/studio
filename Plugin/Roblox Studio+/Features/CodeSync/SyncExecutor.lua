...
return (function(storageFolder)	
	...
	local function isMapping(mappingInstance)
		...
		if (mappedDirectory
			and mappedDirectory:IsA("StringValue")
			and mappedDirectory.Value ~= "" 
			and mappingInstance.Value 
			and mappingInstance.Value.Parent
			and lastUpdated
			and lastUpdated:IsA("NumberValue")) then
			return true
		end
